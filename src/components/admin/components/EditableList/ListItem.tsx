...
function EditableListItem({
  ...
  return (
    <tr className="hover:bg-slate-100 dark:hover:bg-slate-100 hover:text-slate-800 dark:text-slate-50 dark:hover:text-slate-800">
      <th scope="row" className="px-4 py-3 font-medium whitespace-nowrap">{title}</th>
      <td className="px-4 py-3">{description}</td>
      <td className="px-4 py-3">{imageUrl || ''}</td>
      <td className="px-4 py-3 flex items-center justify-end relative">
        <button
          ...
          onClick={(evt) => {
            evt.preventDefault();
            if (onSelect) {
              onSelect(!showDropdown ? id : undefined);
            }
          }}
        >
        ...
        <div className="relative">
          <div className={`${showDropdown ? '' : 'hidden'} top-6 right-0 absolute z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600`}>
